...
const AssignmentListPage = async ({
  ...
  const renderRow = async (item: AssignmentsList) => (
    <tr
      key={item.id}
    ...
    >
      ...
      <td className="hidden md:table-cell">{`${item.lesson.teacher.surname} ${
        item.lesson.teacher.name[0]
      }. ${item.lesson.teacher.patronymic?.[0] ?? ""}.`}</td>
      <td className="hidden md:table-cell">
        {new Intl.DateTimeFormat("ru-Ru").format(item.dueDate)}
      </td>
      <td>
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Задания</h1>
